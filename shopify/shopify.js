...
async function callShopify(query) {
  const fetchUrl = `https://goodhod.myshopify.com/api/2021-01/graphql.json`;

  const fetchOptions = {
    endpoint: fetchUrl,
    method: "POST",
    headers: {
      "X-Shopify-Storefront-Access-Token": "544825cddbe1e6f538df09d18374ac6e",
...
export async function getAllProductsInCollection() {
  const query = `{
      collectionByHandle(handle: "homes") {
        id
        title
        products(first: 250) {
          edges {
